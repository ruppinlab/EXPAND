...
# Date: Jul 15, 2025
# Title: HIFs-based CoxPH Survival Analysis for BRCA Subtypes (HER2+, HR+, TNBC)
# Models: (2) All HIFs + Age (collinearity removal before CV, fold-wise normalization, Age not scaled)
# -----------------------------------------------------------------------------------------------------------------------
...
data_file = f"{dataset_name}/outputs/PathAI/Survival_V2/BrcaBiomarkerStatusToPathAI_HIFsToSurvival.csv"
output_dir = os.path.join("/data/Ruppin_AI/BRCA_PIF/Ranjan/DataForFigures/Survival_V2", "HIFs")
os.makedirs(output_dir, exist_ok=True)

# Load data
data = pd.read_csv(data_file)

# Define HIFs (607 features)
HIFs = data.columns[4:611].tolist()
print("Total number of HIFs:", len(HIFs))
...
# Fixed collinearity filter function with threshold=0.65
def remove_highly_correlated_features(df, threshold=0.65):
    corr_matrix = df.corr().abs()
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
    drop_cols = [col for col in upper.columns if any(upper[col] > threshold)]
    reduced_df = df.drop(columns=drop_cols)
    return reduced_df.columns.tolist()
...
her2_hifs = remove_highly_correlated_features(her2[HIFs], threshold=0.65)
print(f"[HER2+ | Pre-CV] Fixed threshold = 0.65, Features retained = {len(her2_hifs)}")
print(f"[HER2+ | Retained Features] {her2_hifs}\n")

hr_hifs = remove_highly_correlated_features(hr[HIFs], threshold=0.65)
print(f"[HR+ | Pre-CV] Fixed threshold = 0.65, Features retained = {len(hr_hifs)}")
print(f"[HR+ | Retained Features] {hr_hifs}\n")

tnbc_hifs = remove_highly_correlated_features(tn[HIFs], threshold=0.65)
print(f"[TNBC | Pre-CV] Fixed threshold = 0.65, Features retained = {len(tnbc_hifs)}")
print(f"[TNBC | Retained Features] {tnbc_hifs}\n")



# Run survival analysis with filtered HIFs
her2_os_df   = coxph_5fold_cv_combined_km(her2, 'HER2+', her2_hifs, 'os', output_dir)
her2_pfs_df  = coxph_5fold_cv_combined_km(her2, 'HER2+', her2_hifs, 'pfs', output_dir)

hr_os_df     = coxph_5fold_cv_combined_km(hr, 'HR+', hr_hifs, 'os', output_dir)
hr_pfs_df    = coxph_5fold_cv_combined_km(hr, 'HR+', hr_hifs, 'pfs', output_dir)

tnbc_os_df   = coxph_5fold_cv_combined_km(tn, 'TNBC', tnbc_hifs, 'os', output_dir)
tnbc_pfs_df  = coxph_5fold_cv_combined_km(tn, 'TNBC', tnbc_hifs, 'pfs', output_dir)
...
final_csv = os.path.join(output_dir, "All_Subtypes_AllSurvival_Combined_TestSamples_HIFs.csv")
