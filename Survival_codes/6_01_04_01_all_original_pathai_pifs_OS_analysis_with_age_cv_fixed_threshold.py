...
# Date: Jul 15, 2025
# Title: PIFs-based CoxPH Survival Analysis for BRCA Subtypes (HER2+, HR+, TNBC)
# Models: (2) All PIFs + Age (collinearity removal before CV, fold-wise normalization, Age not scaled)
# -----------------------------------------------------------------------------------------------------------------------
...
data_file = f"{dataset_name}/outputs/PathAI/Survival_V2/BrcaBiomarkerStatusToPathAI_PIFsToSurvival.csv"
output_dir = os.path.join("/data/Ruppin_AI/BRCA_PIF/Ranjan/DataForFigures/Survival_V2", "PIFs")
...
# Define PIFs
PIFs = data.columns[4:29].tolist()
print("Total number of PIFs:", len(PIFs))
PIFs
...
# Fixed-threshold collinearity filter function (threshold = 0.9)
def remove_highly_correlated_features_fixed(df, threshold=0.9):
    corr_matrix = df.corr().abs()
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
    drop_cols = [col for col in upper.columns if any(upper[col] > threshold)]
    reduced_df = df.drop(columns=drop_cols)
    return reduced_df.columns.tolist(), threshold
...
# Apply collinearity filtering before CV per subtype
her2_pifs, her2_thresh = remove_highly_correlated_features_fixed(her2[PIFs], threshold=0.9)
print(f"[HER2+ | Pre-CV] Threshold = {her2_thresh:.2f}, Features retained = {len(her2_pifs)}")
print(f"[HER2+ | Retained Features] {her2_pifs}\n")

hr_pifs, hr_thresh = remove_highly_correlated_features_fixed(hr[PIFs], threshold=0.9)
print(f"[HR+ | Pre-CV] Threshold = {hr_thresh:.2f}, Features retained = {len(hr_pifs)}")
print(f"[HR+ | Retained Features] {hr_pifs}\n")

tnbc_pifs, tnbc_thresh = remove_highly_correlated_features_fixed(tn[PIFs], threshold=0.9)
print(f"[TNBC | Pre-CV] Threshold = {tnbc_thresh:.2f}, Features retained = {len(tnbc_pifs)}")
print(f"[TNBC | Retained Features] {tnbc_pifs}\n")

# Run survival analysis with filtered PIFs
her2_os_df   = coxph_5fold_cv_combined_km(her2, 'HER2+', her2_pifs, 'os', output_dir)
her2_pfs_df  = coxph_5fold_cv_combined_km(her2, 'HER2+', her2_pifs, 'pfs', output_dir)

hr_os_df     = coxph_5fold_cv_combined_km(hr, 'HR+', hr_pifs, 'os', output_dir)
hr_pfs_df    = coxph_5fold_cv_combined_km(hr, 'HR+', hr_pifs, 'pfs', output_dir)

tnbc_os_df   = coxph_5fold_cv_combined_km(tn, 'TNBC', tnbc_pifs, 'os', output_dir)
tnbc_pfs_df  = coxph_5fold_cv_combined_km(tn, 'TNBC', tnbc_pifs, 'pfs', output_dir)
...
final_csv = os.path.join(output_dir, "All_Subtypes_AllSurvival_Combined_TestSamples_PIFs.csv")
